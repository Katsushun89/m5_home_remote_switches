...
private:
    ...
    static constexpr uint32_t RING_OUTSIDE_WIDTH = 6;
    static constexpr uint32_t RING_TOTAL_WIDTH = RING_OUTSIDE_WIDTH+RING_INSIDE_WIDTH;
    static constexpr uint8_t RING_INSIDE_DIV = 6;

    void drawLRButton(void);
    void drawCenterBase(void);
    void drawCenterTransitionOff2On(uint32_t keep_push_time, uint32_t decision_time);
    void drawCenterTransitionOn2Off(uint32_t keep_push_time, uint32_t decision_time);
...
public:
    ...
    void drawCenter(ButtonStatus &status, uint32_t decision_time, String current_switch_str);
    static constexpr int32_t CENTER_BUTTON_WIDTH = 239;
};